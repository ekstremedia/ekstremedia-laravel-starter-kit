...
<template>
    <div class="flex flex-col h-full">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-dark-700">
            ...
            <button
                @click="emit('newChat')"
                class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-dark-800 transition-colors cursor-pointer focus-visible:ring-2 focus-visible:ring-indigo-500"
        ...
        <ul v-else class="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-dark-800">
            <li v-for="c in conversations" :key="c.id">
                <button
                    type="button"
                    @click="emit('select', c)"
                    class="w-full flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-dark-800/50 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-500"
